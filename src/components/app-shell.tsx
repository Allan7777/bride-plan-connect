...
const vendorLinks = [
  ...
  { to: "/painel/perfil", label: "Perfil e portfólio", icon: UserRound },
  { to: "/painel/leads", label: "Leads", icon: UserRound },
  { to: "/painel", label: "Estatísticas", icon: BarChart3 },
  { to: "/assinatura", label: "Assinatura", icon: WalletCards },
  { to: "/painel/perfil", label: "Configurações", icon: Settings },
] as const;
...
export function AppShell({ type, children }: { type: "bride" | "vendor"; children: React.ReactNode }) {
  ...
  return <div className={vendor ? "min-h-screen bg-slate-50 text-slate-950" : "min-h-screen bg-background"}>
    <aside className={vendor ? "fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-800 bg-slate-950 p-6 text-white lg:block" : "fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-border bg-sidebar p-5 lg:block"}>
      <Link to={vendor ? "/painel" : "/"} className={vendor ? "block text-2xl font-extrabold tracking-tight" : "font-display text-2xl"}>
        {vendor ? <>NoivaHub <span className="text-emerald-400">Pro</span></> : <>Noiva<span className="text-gold">Hub</span></>}
      </Link>
      {vendor && <p className="mt-2 text-sm text-slate-400">Área do Fornecedor</p>}
      <nav className={vendor ? "mt-10 space-y-1" : "mt-10 space-y-1"}>
        {links.map((item,index) => <Link key={item.label+index} to={item.to} activeOptions={{ exact: true }}
